...
class CrossCompileScript:
	...
	def loadPackages(self, packages_folder):
		...
		for path, subdirs, files in os.walk(prodFolder):
			for name in files:
				p = Path(os.path.join(path, name))
				if p.suffix == ".py":
					self.logger.debug("h3333 Found Product '%s' ." % (p))
					if not isPathDisabled(p):
						tmpPkglist["prods"].append(p)
						self.logger.debug("h3333 tmp Loaded Product '%s' ." % (p))
					else:
						self.logger.debug("h3333 tmp DID NOT Load Product '%s' due to isPathDisabled." % (p))
			...
			with open(d, "r", encoding="utf-8") as f:
				...
				try:
					...
					if self.boolKey(o, "_disabled"):
						...
						self.logger.debug("h3333 Loaded Product '%s' ." % (p))
	...
	def prepareBuilding(self, bitness):
		...
		# no, not here
		# this, instead, needs to be decided at time of building the product or dependency
		# since it may be in a product or offtree folder or a dependency folder
		# which may have conflicting (duplicate) folder names
