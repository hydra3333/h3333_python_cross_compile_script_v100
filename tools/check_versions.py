...
import subprocess
import sys
from telnetlib import EC # 2022.12.18 per DEADSIX27
import traceback
...
#for name, pkg in enumerate(sorted(pkgs["prods"].items(),key=lambda i: i[0].casefold())):
	print(Fore.GREEN +  "Checking package %s ..." % (name + Style.RESET_ALL))
	...
	if "update_check" in pkg:
		...
		if vType == "git":  # packages that are git clones
			...
			#latestVer = geLatestVersion(versionEl)		# 2022.12.18 per DEADSIX27
			try:										# 2022.12.18 per DEADSIX27
				latestVer = geLatestVersion(versionEl)	# 2022.12.18 per DEADSIX27
			except Exception:							# 2022.12.18 per DEADSIX27
...
#for name, pkg in enumerate(sorted(pkgs["deps"].items(),key=lambda i: i[0].casefold())):
	print(Fore.GREEN +  "Checking dependency %s ..." % (name + Style.RESET_ALL))
	if specificPkgs is not None:
		if not any(word in name for word in specificPkgs):
			continue
