{ # 2019.12.13 # https://git.gnupg.org/cgi-bin/gitweb.cgi?p=libgcrypt.git;a=summary # This DAMN thing fails to build with "versioninfo.rc.in:21: syntax error" if not built directly from a GIT clone
  # see https://forum.videolan.org/viewtopic.php?f=32&t=127218&p=432818&hilit=compile+libaacs#p432818
	...
	#'configure_options': '--host={target_host} --prefix={output_prefix}/libaacs_dll_git.installed --with-gpg-error-prefix={output_prefix}/libaacs_dll_git.installed --disable-doc '
	#			'--enable-ciphers=aes --enable-pubkey-ciphers=ecc --enable-digests=sha1 --enable-digests=sha256  --enable-digests=sha512  --enable-random=w32 '
	#			'--enable-kdfs=pkdf2 --disable-padlock-support --enable-threads=windows --disable-asm ',
