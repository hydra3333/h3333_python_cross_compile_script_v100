...
import logging
#import os.path
import os # 2020.05.20 import full os since we want mkdir
import re
import shutil
import stat
import subprocess
...
class CrossCompileScript:
	...
	def loadPackages(self, packages_folder):
		...
		for path, subdirs, files in os.walk(prodFolder):
			for name in files:
				p = Path(os.path.join(path, name))
				if p.suffix == ".py":
					self.logger.debug("h3333 tmp parsing files, Found Product '%s' ." % (p))
					if not isPathDisabled(p):
						tmpPkglist["prods"].append(p)
						self.logger.debug("h3333 tmp parsing files, Loaded Product '%s' ." % (p))
					else:
						self.logger.debug("h3333 tmp parsing files, did NOT Load Product '%s' due to isPathDisabled." % (p))
	...
	def loadConfig(self):
		self.config = {  # Default config
			...
			'script': {
				...
				#'mingw_toolchain_path': 'mingw_toolchain_script/mingw_toolchain_script_v100_001.py',  # 2022.12.18 USE A FIXED FILENAME FOR BUILDING MINGw
				'mingw_toolchain_path': 'mingw_toolchain_script/mingw_toolchain_script_v100_002_like_zeranoe.py',  # 2022.12.18 USE A FIXED FILENAME FOR BUILDING MINGw
		...
		#            This is to mitigate changes in mingw commits specified here in this script,
		#            which were invisibly overridden by the older .yaml when we don't want them to be.
	...
	def commandLineEntrace(self):
		...
		_epilog = 'Copyright (C) 2018-2023 DeadSix27 (https://github.com/DeadSix27/python_cross_compile_script)\n\n This Source Code Form is subject to the terms of the Mozilla Public\n License, v. 2.0. If a copy of the MPL was not distributed with this\n file, You can obtain one at https://mozilla.org/MPL/2.0/.\n '
	...
	def prepareBuilding(self, bitness):
		...
		self.fullWorkDir_COPY = self.fullWorkDir
		self.mingwDir_COPY = self.mingwDir # FOR PRINTING
		self.bitnessStr_COPY = self.bitnessStr
		self.bitnessStr_COPY_WITH_STRING = self.bitnessStr_COPY + "-w64-mingw32"
		self.targetHostStr_COPY = self.targetHostStr
		self.targetPrefix_COPY_WITH_ABOVE = self.targetPrefix
...
#
		...
		print(f"self.fullWorkDir_COPY='{self.fullWorkDir_COPY}'")
		print(f"self.mingwDir_COPY='{self.mingwDir_COPY}'")
		print(f"self.bitnessStr_COPY='{self.bitnessStr_COPY}'")
		print(f"self.bitnessStr_COPY_WITH_STRING='{self.bitnessStr_COPY_WITH_STRING}'")
		print(f"self.targetHostStr_COPY='{self.targetHostStr_COPY}'")
		print(f"self.targetPrefix_COPY_WITH_ABOVE='{self.targetPrefix_COPY_WITH_ABOVE}'")
		
		print(f"self.inTreePrefix='{self.inTreePrefix}'")
		print(f"self.offtreePrefix='{self.offtreePrefix}'")
		print(f"self.targetSubPrefix='{self.targetSubPrefix}'")
		print(f"self.mingwBinpath='{self.mingwBinpath}'")
		print(f"self.mingwBinpath2='{self.mingwBinpath2}'")
		print(f"self.fullCrossPrefixStr='{self.fullCrossPrefixStr}'")
		print(f"self.shortCrossPrefixStr='{self.shortCrossPrefixStr}'")
		print(f"self.autoConfPrefixOptions='{self.autoConfPrefixOptions}'")
		print(f"self.makePrefixOptions='{self.makePrefixOptions}'")
		print(f"self.pkgConfigPath='{self.pkgConfigPath}'")
		print(f"self.localPkgConfigPath='{self.localPkgConfigPath}'")
		print(f"self.mesonEnvFile='{self.mesonEnvFile}'")
		print(f"self.cmakeToolchainFile='{self.cmakeToolchainFile}'")
		print(f"self.cmakePrefixOptions='{self.cmakePrefixOptions}'")
		print(f"self.cmakePrefixOptionsOld='{self.cmakePrefixOptionsOld}'")
		print(f"self.cpuCount='{self.cpuCount}'")
		print(f"self.original_stack_protector='{self.original_stack_protector}'")
		print(f"self.original_stack_protector_trim='{self.original_stack_protector_trim}'")
		print(f"self.original_fortify_source='{self.original_fortify_source}'")
		print(f"self.original_fortify_source_trim='{self.original_fortify_source_trim}'")
		print(f"self.originalCflag='{self.originalCflag}'")
		print(f"self.originalCflag_trim='{self.originalCflag_trim}'")
		print(f"self.originalCflags='{self.originalCflags}'")
		print(f"self.originalCflags_trim='{self.originalCflags_trim}'")
		print(f"self.originalLdLibPath='{self.originalLdLibPath}'")
	...
	def createCmakeToolchainFile(self):
		if not os.path.isfile(self.cmakeToolchainFile):
			...
			tcFile = [
				...
				#F'set(CMAKE_SYSROOT {self.targetSubPrefix})', # 2022.12.18 per DEADSIX27
				...
				#F'set(CMAKE_FIND_ROOT_PATH {self.targetPrefix})', # 2022.12.18 per DEADSIX27
				F'set(CMAKE_FIND_ROOT_PATH {self.targetPrefix}/)', # 2022.12.18 per DEADSIX27
				...
				#F'set(MINGW_SYSROOT {self.targetSubPrefix})'  # 2022.12.18 per DEADSIX27
			]
	...
	def createMesonEnvFile(self):
		if not os.path.isfile(self.mesonEnvFile):
			...
			meFile = (
				...
				"",
				"[properties]",
				#"c_link_args = ['-static', '-static-libgcc']", # 2022.09.26 see below
				"# sys_root = Directory that contains 'bin', 'lib', etc for the toolchain and system libraries",
				#F"sys_root = '{self.targetSubPrefix}'" # 2022.12.18 per DEADSIX27
				"",
				"[built-in options]",
	...
	def runProcess(self, command, ignoreErrors=False, exitOnError=True, silent=False):
		...
		while True:
			...
			if isSvn:
				#if not nextline.decode('utf-8').startswith('A    '):  # 2022.12.18 per DEADSIX27
		...
		if os.path.isdir(realFolderName):
			if desiredPR is not None:
				...
				if LOCAL == REMOTE:
					...
					self.logger.debug("LOCAL:  " + LOCAL)
					self.logger.debug("REMOTE: " + REMOTE)
					#self.logger.debug("BASE    " + BASE) # 2022.12.18 per DEADSIX27
					self.logger.debug("BASE	" + BASE)
					self.logger.debug("####################")
				self.cchdir("..")
		else:
	...
	def buildThing(self, packageName, packageData, type, forceRebuild=False, skipDepends=False):  # type = PRODUCT or DEPENDENCY # I couldn't come up with a better name :S
		...
		if 'cflag_addition' in packageData:
			if packageData['cflag_addition'] is not None:
				...
				self.logger.debug(F'os.environ CFLAGS   before cflag_addition = "{os.environ["CFLAGS"]}') # 2019.12.13
				self.logger.debug(F'os.environ CXXFLAGS before cflag_addition = "{os.environ["CXXFLAGS"]}') # 2019.12.13
				self.logger.debug(F'os.environ CPPFLAGS before cflag_addition = "{os.environ["CPPFLAGS"]}') # 2019.12.13
				self.logger.debug(F'os.environ LDFLAGS  before cflag_addition = "{os.environ["LDFLAGS"]}') # 2019.12.13
		...
		if 'custom_cflag' in packageData:
			if packageData['custom_cflag'] is not None:
				...
				self.logger.info(F'Set custom CFLAGS,   now: "{os.environ["CFLAGS"]}"')
				self.logger.info(F'Set custom CXXFLAGS, now: "{os.environ["CXXFLAGS"]}"')
				self.logger.info(F'Set custom CPPFLAGS, now: "{os.environ["CPPFLAGS"]}"')
				self.logger.info(F'Set custom LDFLAGS,  now: "{os.environ["LDFLAGS"]}"')
				if self.debugMode:
					print("##############################")
					#print("### Environment variables:  ###")
					#for tk in os.environ:
		...
		if not self.anyFileStartsWith('already_ran_make'):
			...
			# 2020.04.08 comment-out ex deadsix27 https://github.com/DeadSix27/python_cross_compile_script/commit/2e3be3b7642932a40a95614146de50080aaed79c
			...
			# 2020.04.08 added here for shaderc per deadsix27 https://github.com/DeadSix27/python_cross_compile_script/commit/2e3be3b7642932a40a95614146de50080aaed79c
			if 'regex_replace' in packageData and packageData['regex_replace']:
				_pos = 'post_patch'
				if isinstance(packageData['regex_replace'], dict) and _pos in packageData['regex_replace']:
					for r in packageData['regex_replace'][_pos]:
	...
	def mesonSource(self, packageName, packageData):
		...
		if not os.path.isfile(touchName):
			...
			#self.logger.debug('meson {0}'.format(makeOpts)) # 2022.12.18 per DEADSIX27
			#self.runProcess('meson {0}'.format(makeOpts))   # 2022.12.18 per DEADSIX27
			self.logger.debug('meson setup {0}'.format(makeOpts)) # 2022.12.18 per DEADSIX27
			self.runProcess('meson setup {0}'.format(makeOpts))   # 2022.12.18 per DEADSIX27
